...
// Local calendar date (YYYY-MM-DD). Avoid toISOString(): it converts to UTC and
// silently shifts the day for any user east of Greenwich.
...
export default function DateRangePicker({
  ...
  const [openAt, setOpenAt] = useState<OpenAt>(null);
  ...
  useEffect(() => {
    ...
    const onDoc = (e: MouseEvent) => {
      if (!wrapRef.current?.contains(e.target as Node)) setOpenAt(null);
    };
  ...
  useEffect(() => {
    ...
    if (range?.from && range?.to && range.from.getTime() !== range.to.getTime()) {
      const t = setTimeout(() => setOpenAt(null), 250);
    ...
    >
      <DayPicker
        ...
        classNames={{
          ...
          weekday: 'text-[10px] uppercase tracking-[0.16em] text-muted-soft font-bold',
          nav_button:
  ...
  // Per-variant class for each trigger cell (which wraps a TriggerCell + owns the popover anchor point).
  let cellWrapperClass: string;
  let triggerCellClass: string;
  let containerClass: string;
  let divider: ReactNode = null;

  if (variant === 'hero') {
    containerClass = 'flex flex-col md:flex-row gap-3 md:items-stretch flex-1';
    cellWrapperClass = 'relative flex-1';
    triggerCellClass = triggerBase + ' w-full px-3 py-1 hover:bg-bone/60';
    divider = <div className="hidden md:block w-px self-stretch bg-sand" />;
  } else if (variant === 'stack') {
    containerClass = 'grid grid-cols-2 gap-3';
    cellWrapperClass = 'relative';
    triggerCellClass =
      triggerBase +
      ' w-full bg-bone border border-sand rounded-lg px-3 py-2 hover:border-clay ' +
      (open ? 'border-clay' : '');
  } else {
    containerClass = 'grid grid-cols-2 gap-3';
    cellWrapperClass = 'relative';
    triggerCellClass =
      triggerBase +
      ' w-full bg-bone border border-sand rounded-lg px-4 py-3 hover:border-clay ' +
      (open ? 'border-clay' : '');
  }
  ...
  return (
    <div ref={wrapRef} className="relative flex-1 w-full">
      <div className={containerClass}>
        <div className={cellWrapperClass}>
          <TriggerCell
            ...
            onClick={() => setOpenAt('from')}
            className={triggerCellClass}
            active={openAt === 'from'}
          />
          {openAt === 'from' && popover}
        </div>
      ...
      {name && (
        <>
          <input type="hidden" name={`${name}From`} value={toISODate(range?.from)} />
          <input type="hidden" name={`${name}To`} value={toISODate(range?.to)} />
        </>
      )}
    </div>
  );
